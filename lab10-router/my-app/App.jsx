...
function App() {
  ...
  */
  ...
  function deleteFilm(id) {
    setFilmList( filmList => filmList.filter( film => film.id !== id));
  }
  ...
  function addFilm(newFilm) {
    setFilmList( (films) => {
      const id = Math.max(...(filmList.map((film) => film.id))) + 1;
      return [...films, {"id": id, ...newFilm}]; 
    });
  ...
  return (
    <Container fluid>
      ...
      <Row>
        <Col xs={3}>
          <Filters items={filterArray} selected={activeFilter} setSelected={setActiveFilter} />
        </Col>

        <Col xs={9}>
          <div className="d-flex flex-row justify-content-between">
            <h1 className="my-2"><span>{filters[activeFilter].label}</span></h1>
            {showForm ? null : <Button variant="primary" className="my-2" onClick={()=>setShowForm(true)}>&#43;</Button>}
          </div>
